...
def eval_once(var_init, summary_writer, saver, all_summaries, best_MAE, best_epoch, labels, logits, validation, phase_train):
        ...
        with slim.queues.QueueRunners(mon_sess):
            ...
            for z in range(FLAGS.batch_size):
                ...
                if serz[z] in data:
                    ...
                    data[serz[z]] = {'label': lbl1[z], 'log1': [logtz[z]], 'tot': 1, 'avg': None}
            ...
            sdt.calculate_metrics(np.squeeze(np.asarray(logga)), np.squeeze(np.asarray(labba)), 0, 1, True)
    ...
    return Epoch, best_MAE, best_epoch
...
def eval():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        while True:
            ...
            # Check folder for changes then run once
            filecheck = glob.glob(FLAGS.train_dir + FLAGS.RunInfo + '*')
            Epoch, best_MAE, best_epoch = eval_once(var_init, summary_writer, saver, all_summaries, best_MAE, best_epoch, labels, logits, validation, phase_train)

            # Break if this is the final checkpoint
            if int(Epoch) > 1350: break
